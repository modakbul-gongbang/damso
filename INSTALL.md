...
`search_meetings`/`search_people` default to 20 results and cap at 100 (`limit`), with `offset` for paging past the first batch; `search_meetings` also takes `from`/`to` for an inclusive date range as an alternative to the single-value `date` filter (the two cannot be combined). A malformed `date`/`from`/`to`, an out-of-range `limit`/`offset`, or a `get_meeting`/`get_speaker` lookup that matches nothing returns an MCP tool error (`isError: true`) with a human-readable reason instead of a bare empty result or `null` - the previous behavior made a typo indistinguishable from a genuinely empty answer.
`get_meeting` returns the meeting's actual transcript segments (speaker/text/timing), with any confirmed cleanup corrections from `transcript.cleaned.json` already applied when they still match the transcript's generation; earlier versions of this tool always returned an empty transcript regardless of what had been transcribed.
`get_speaker`'s `meetings` list is restricted to an exact match on the resolved participant identity, unlike `search_meetings`'s deliberately loose `speaker` substring filter - a `get_speaker` call for a common short name no longer picks up unrelated meetings that merely contain it as a substring.
Every tool parameter carries a description in its JSON schema, and the `initialize` response includes `instructions` describing the intended search-then-read workflow (find a `stem`/`name` with `search_meetings`/`search_people`, then read it with `get_meeting`/`get_speaker`) for MCP clients that read it.
Phase-one and summary processing each run in their own subprocess of the daemon rather than inside the daemon's own process, so a long-running transcription or diarization job no longer holds Python's GIL for its duration and makes `/v1` and `/mcp` unresponsive while it runs.
